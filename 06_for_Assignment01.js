console.log(`========== WAP to print numbers from 5 to 15 incrementing 1 ==========`);
for (let index = 5; index<=15 ; index++) {
    console.log(index); 
}

console.log(`========== WAP to print numbers from 50 to 40 decrementing by 1 ==========`);
for (let index = 50; index >=40; index--) {
    console.log(index);
}

console.log(`========== WAP to find 15 odd number ==========`);
for (let index = 1; index <=30; index=index+2) {
    console.log(index);
}

console.log(`========== WAP to find first 10 even number ==========`);
for (let index = 0; index <=20; index=index+2) {
    console.log(index);
}

    console.log(`========== WAP to print table of 5 like --> 5 10 15...50 ===========`);
    for (let index = 5; index <=50; index=index+5) {
        console.log(index);
    }

    console.log(`========== WAP to print table of 10 like 10 20 30....100 =========`);
    for (let index = 10; index <=100; index=index+10) {
        console.log(index);
    }

    console.log(`WAP to print table of 10 in reverse order like --> 100 90 80...10`);
    for (let index = 100; index >=10; index=index-10) {
        console.log(index)
    }
